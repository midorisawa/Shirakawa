...
test('編集とリスト操作は保存ボタンまで反映せず、保存時にAPIキー確認を再送しない', async () => {
  ...
  try {
    ...
    const optionsJs = readFileSync(new URL('../src/options/index.js', import.meta.url), 'utf8');
    ...
    assert.match(optionsCss, /#saveState:not\(\.feedback-visible\), #status:not\(\.feedback-visible\) \{ display: none; \}/);
    assert.match(optionsJs, /duration: 110, easing: 'ease', fill: 'both'/);
    assert.match(optionsJs, /setAttribute\('aria-hidden', 'true'\)/);
    ...
    const originalFeedbackAnimate = dom.window.HTMLElement.prototype.animate;
    dom.window.HTMLElement.prototype.animate = function (frames, options) {
      feedbackAnimations.push({ element: this, frames, options });
    ...
    assert.equal(feedbackAnimations.length, 1);
    assert.equal(feedbackAnimations[0].options.duration, 110);
    modelInput.dispatchEvent(new dom.window.Event('input', { bubbles: true }));
    assert.equal(feedbackAnimations.length, 1);
    ...
    assert.equal(feedbackAnimations.length, 2);
    modelInput.value = 'changed-model';
    modelInput.dispatchEvent(new dom.window.Event('input', { bubbles: true }));
    ...
    assert.equal(document.getElementById('save').disabled, true);
    assert.equal(document.getElementById('closeWithoutSaving').disabled, true);
    if (originalFeedbackAnimate === undefined) delete dom.window.HTMLElement.prototype.animate;
    else dom.window.HTMLElement.prototype.animate = originalFeedbackAnimate;
    ...
    const keyAnimations = [];
    const originalKeyAnimate = dom.window.HTMLElement.prototype.animate;
    const originalMatchMedia = dom.window.matchMedia;
    dom.window.HTMLElement.prototype.animate = function (frames, options) { keyAnimations.push({ element: this, frames, options }); return { finished: Promise.resolve(), cancel() {} }; };
    ...
    const keyView = document.getElementById('apiKeyRow').closest('.connection-grid');
    assert.equal(keyAnimations.at(-1).element, keyView);
    assert.equal(keyAnimations.at(-1).options.duration, 180);
    ...
    assert.equal(document.getElementById('keyStatus').textContent, '保存済み');
    const animationsBeforeReducedMotion = keyAnimations.length;
    dom.window.matchMedia = () => ({ matches: true });
    document.getElementById('changeApiKey').click();
    document.getElementById('cancelApiKey').click();
    assert.equal(keyAnimations.length, animationsBeforeReducedMotion);
    if (originalMatchMedia === undefined) delete dom.window.matchMedia; else dom.window.matchMedia = originalMatchMedia;
    if (originalKeyAnimate === undefined) delete dom.window.HTMLElement.prototype.animate; else dom.window.HTMLElement.prototype.animate = originalKeyAnimate;
    ...
    document.querySelector('[data-group="black"] [data-edit-rule]').click();
    const rowsBeforeAdd = [...document.querySelectorAll('#rules [data-group]')];
    const animationsBeforeAdd = additionAnimations.length;
    const originalAnimate = dom.window.HTMLElement.prototype.animate;
    dom.window.HTMLElement.prototype.animate = function (frames, options) {
      additionAnimations.push({ element: this, frames, options });
      return { finished: Promise.resolve(), cancel() {} };
    };
    ...
    assert.equal(additionAnimations.length, animationsBeforeAdd + 1);
    assert.equal(additionAnimations.at(-1).element, newRow);
    assert.ok(rowsBeforeAdd.every(row => !additionAnimations.some(animation => animation.element === row)));
    assert.deepEqual(additionAnimations.at(-1).options, { duration: 180, easing: 'ease-out', fill: 'both' });
    assert.equal(additionAnimations.at(-1).frames[0].height, '0px');
    assert.equal(additionAnimations.at(-1).frames[0].paddingBlock, '0px');
    assert.equal(additionAnimations.at(-1).frames[0].marginBlock, '0px');
    await new Promise(resolve => setTimeout(resolve, 0));
...
test('条件の削除アニメーション後も行がDOMと保存データから消える', async () => {
  ...
  dom.window.HTMLElement.prototype.animate = function (_frames, options) { animationOptions.push({ element: this, options }); return { finished: new Promise(resolve => setTimeout(resolve, 0)), cancel() {} }; };
  let savedConfig;
  ...
  try {
    ...
    await tick();
    await tick();
    const rowAnimations = animationOptions.filter(animation => animation.element === row);
    assert.equal(rowAnimations[0].options.fill, 'forwards');
    assert.equal(rowAnimations.length, 2);
...
test('初回キー保存と接続先別の保存状態を表示し、不正な設定は保存しない', async () => {
  ...
  try {
    ...
    const providerAnimations = [];
    const originalProviderAnimate = dom.window.HTMLElement.prototype.animate;
    const modelBeforeProviderChange = $('model').value;
    dom.window.HTMLElement.prototype.animate = function (frames, options) { providerAnimations.push({ element: this, frames, options }); return { finished: Promise.resolve(), cancel() {} }; };
    $('provider').value = 'openrouter';
    $('provider').dispatchEvent(new dom.window.Event('change'));
    await tick();
    const animatedProviderElements = providerAnimations.filter(animation => animation.options.duration === 180).map(animation => animation.element);
    assert.ok(animatedProviderElements.includes($('apiKeyRow').closest('.connection-grid')));
    assert.ok(animatedProviderElements.includes($('model')));
    assert.notEqual($('model').value, modelBeforeProviderChange);
    assert.equal($('apiKeyRow').hidden, true);
    assert.equal($('changeApiKey').hidden, false);
    if (originalProviderAnimate === undefined) delete dom.window.HTMLElement.prototype.animate; else dom.window.HTMLElement.prototype.animate = originalProviderAnimate;
