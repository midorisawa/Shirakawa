...
test('削除・リセット操作は保存時まで送信せず、確認キャンセルと部分失敗を保持する', async () => {
  ...
  try {
    ...
    assert.equal(messages.some(message => message.type === 'reset-total-usage'), false);
    ...
    assert.match(confirmMessage, /累計使用量のリセット/);
    ...
    assert.equal(messages.filter(message => message.type === 'reset-total-usage').length, 1);
